...
namespace Loader {
  ...
  struct AssetLoadTask : Tasks::ILongTask {
    ...
    std::shared_ptr<AssetLoadTask> addTask(AppTaskArgs& args, TaskCallback&& subtask);
    static std::shared_ptr<AssetLoadTask> addTask(std::shared_ptr<AssetLoadTask>& head, AppTaskArgs& args, const AssetLoadTaskArgs& deps, TaskCallback&& subtask);
