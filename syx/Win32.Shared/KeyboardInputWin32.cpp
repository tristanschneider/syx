...
KeyState KeyboardInputWin32::getKeyState(const std::string& key) const {
...
bool KeyboardInputWin32::getKeyDown(Key key) const {
  return (mCurState[static_cast<int>(key)] & 128) != 0;
}

bool KeyboardInputWin32::getKeyUp(Key key) const {
  return (mCurState[static_cast<int>(key)] & 128) == 0;
}

bool KeyboardInputWin32::getKeyTriggered(Key key) const {
  return getKeyState(key) == KeyState::Triggered;
}

bool KeyboardInputWin32::getKeyReleased(Key key) const {
  return getKeyState(key) == KeyState::Released;
}

KeyState KeyboardInputWin32::getAsciiState(char c) const {
  switch(c) {
...
KeyState KeyboardInputWin32::_shiftAnd(Key key) const {
  if(getKeyDown(Key::Shift))
    return getKeyState(key);
  return KeyState::Up;
}

KeyState KeyboardInputWin32::_noShift(Key key) const {
  if(getKeyDown(Key::Shift))
    return KeyState::Up;
  return getKeyState(key);
}

KeyState KeyboardInputWin32::_or(KeyState a, KeyState b) const {
...
void KeyboardInputWin32::init() {
  std::memset(mPrevState, 0, sKeyCount);
  std::memset(mCurState, 0, sKeyCount);
}

void KeyboardInputWin32::update(float, IWorkerPool&, std::shared_ptr<Task> frameTask) {
...
void KeyboardInputWin32::uninit() {
}
