...
namespace Lua {
  ...
  void Sandbox::push() {
...

    {
      ...
      int f = lua_getfield(*mState, -1, CHUNK_ID);
      lua_pushvalue(*mState, -2);
      const char* at = lua_typename(*mState, lua_type(*mState, -1));
      const char* fi = lua_typename(*mState, lua_type(*mState, -2));
  ...
  void Sandbox::_createSandbox() {
    ...
    assert(preStackSize == lua_gettop(l) && "Stack shouldn't have changed");
  }
